...
public class CalendarRendererMonth extends CalendarRendererWeek {
	// main colors
	  ...
	  public void draw(Graphics2D g2, DateLookPanel panel) {
	    ...
	    int csn = panel.getNumber_of_slots();
	    ...
	    if (i == 0 || gc.get(GregorianCalendar.DAY_OF_MONTH) == 1) {
	      ...
	      g2.drawString(Converter.gc2monthl(gc).replaceFirst(
	    		  Converter.gc2monthl(gc).substring(0,1), Converter.gc2monthl(gc).substring(0,1).toUpperCase())
	    		  +" - "+gc.get(GregorianCalendar.YEAR), x_pos+2 + space,
	          csh / 2 + font_ascent / 2);
	    }
	      

	      int day_width_ext = day_width * 25 / 24 + 10;  // to render DST-switch-day exact too!
	      ...
	      if (day_width > 16) {
	        ...
	        g2.drawString(gc.get(GregorianCalendar.DAY_OF_MONTH) + "",
	            x_pos+4 + space, (3 * csh) / 2 + font_ascent / 2);
	        if (gc.get(GregorianCalendar.DAY_OF_WEEK) == Calendar.MONDAY && (day_width > 2 * (digit_font_width + small_digit_font_width + space))) {
	          ...
	          g2.drawString(gc.get(GregorianCalendar.WEEK_OF_YEAR) + "",
	              x_pos + day_width - space - small_digit_font_width * 2, (3 * csh) / 2 + font_ascent / 2);
	          g2.setFont(f);
	          g2.setColor(Color.black);
	        }
	      }
	        ...
	        g2.setColor(Color.red);
	        g2.drawString(gc.get(GregorianCalendar.WEEK_OF_YEAR) + "",
	            x_pos - 6 * day_width + space / 2, (3 * csh) / 2 + font_ascent / 2);
	        g2.setColor(Color.black);
	      }
	      ...
	      g2.fillRect(x_pos, 2 * csh, day_width_ext, csh);
	      if (day_width > 16) {
	    	  String dayOfWeek = (Converter.getDayOfWeekWString(gc).replace(Converter.getDayOfWeekWString(gc).substring(0,1), Converter.getDayOfWeekWString(gc).substring(0,1).toUpperCase()).substring(0,2));
	    	  g2.setColor(Color.black);
	    	  g2.drawString(dayOfWeek,
	    			  x_pos +5+ space, (5 * csh) / 2 + font_ascent - font_height / 2);
	      ...
	      if (i != 0 && day_width > 16) {
	    	  // very light color for not hide text 
	  ...
	  public void resized(DateLookPanel panel) {
	    ...
	    day_width = panel.getWidth() * 24 / (int) panel.get_number_of_rendered_hours(); // not exact for DST-switches!
	    // because the two number date doesn't like
